...
./seed.sh          # Seed 19 menu items (5 entradas, 5 principales, 5 postres, 4 bebidas)
```
...
clase-1/1-solo-dominio
  ...
  → clase-1/6-platos-controller          ← clase-2 should start here
    → clase-2/1-base
    ...
    → clase-2/7-buscar-comanda            ← clase-odm should start here
      → clase-odm-inicial
      → clase-odm-injecting-deps
      → clase-odm-repo-platos
      → clase-odm-repos
```

**Current state:**
- `clase-1/*` — **done**. Clean linear sequence, retro-fitted with: split domain (one file per class in `src/domain/`), `clases/clase-1.md` progression doc, `docs.yaml` (platos-only OpenAPI), Swagger UI (`swagger-ui-express` + `yaml` packages), and `README.md`. All branches pushed.
- `clase-2/*` — **not yet rebased**. Currently diverges from an old shared ancestor, not from `clase-1/6-platos-controller`. Needs to be rebased onto `clase-1/6`.
- `clase-odm-*` — **not yet rebased**. Same situation, currently diverges from `clase-2` at an old point.
- `main` — points to `clase-1/1-solo-dominio` (the exercise starting point). `backup-main` preserves the old main.

**Next session task:** Rebase `clase-2/1-base` (and the full `clase-2/*` chain) onto `clase-1/6-platos-controller`, then rebase `clase-odm-*` onto `clase-2/7-buscar-comanda`. The clase-2 steps are well-designed so intermediate fabrication is likely not needed — just rebase and handle conflicts.
...
|--------|----------|--------|
| clase-1 | `clase-1/1-solo-dominio` → `clase-1/6-platos-controller` | Done |
| clase-2 | `clase-2/1-base` → `clase-2/7-buscar-comanda` | Needs rebase onto clase-1/6 |
| clase-odm | `clase-odm-inicial` → `clase-odm-repos` | Needs rebase onto clase-2/7 |

Legacy flat branches (`clase-1-solo-dominio`, `clase-1-crear-plato`, `clase-1-rebuild`, `clase-2/base`, `clase-odm`, etc.) exist but are not part of the active teaching sequence.
